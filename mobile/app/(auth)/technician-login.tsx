// ============================================================
// Tela de login do Portal do Técnico.
// Autentica via POST /api/technician-login e salva o JWT no
// SecureStore (cofre criptografado do celular).
// ============================================================
...
import { saveTechnicianSession } from "@/lib/auth";
...
export default function TechnicianLoginScreen() {
  ...
  async function handleLogin() {
    ...
    setLoading(true);
    try {
      const res = await fetch(`${API_URL}/api/technician-login`, {
        method: "POST",
      ...
      // O campo "jwt" é o token real — salvo no SecureStore para usar nas próximas requisições
      if (!data.jwt) {
        Alert.alert("Erro", "Servidor não retornou token. Contate o suporte.");
        return;
      }

      await saveTechnicianSession(data.jwt, data.technicianId, data.name);

      // Substitui o histórico de navegação para que o botão "voltar"
      // não leve o técnico de volta para a tela de login
        ...
        >
          <View className="flex-1 px-6 py-10 justify-center gap-8">
            ...
            <View className="gap-1">
              <Text className="text-3xl font-bold text-gray-900">Portal do Técnico</Text>
